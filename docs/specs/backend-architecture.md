...
```
Balenthiran.Snipit/
├── Balenthiran.Snipit.WebApi         # Entry point: routes, DI, OpenAPI, middleware
...
├── Balenthiran.Snipit.Services       # Business logic implementations
│   └── Mapper.cs               # EntityModel ↔ DomainModel mappings
├── Balenthiran.Snipit.Abstractions   # Interfaces only — no implementations
...
├── Balenthiran.Snipit.EntityModels   # Database entity classes (anemic POCOs)
├── Balenthiran.Snipit.DomainModels   # Rich business-layer objects
└── Balenthiran.Snipit.DataModels     # Request/Response DTOs (public API contract)
```
