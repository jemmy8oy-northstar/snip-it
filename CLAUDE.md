...
│   ├── Balenthiran.Snipit.WebApi/       # Entry point: routes, DI, OpenAPI
│   ├── Balenthiran.Snipit.Services/     # Business logic, AutoMapper profiles
│   ├── Balenthiran.Snipit.Abstractions/ # Service interfaces + model interfaces
│   ├── Balenthiran.Snipit.Database/     # EF Core DbContext + migrations
│   ├── Balenthiran.Snipit.EntityModels/ # Database entities (anemic POCOs)
│   ├── Balenthiran.Snipit.DomainModels/ # Rich business-layer objects
│   └── Balenthiran.Snipit.DataModels/   # Request/Response DTOs
...
```bash
# Backend (Terminal 1)
cd backend && dotnet run --project Balenthiran.Snipit.WebApi
...
cd backend && dotnet build Balenthiran.Snipit.WebApi -c Debug   # refreshes committed openapi.json
...
cd backend && dotnet ef database update \
  --project Balenthiran.Snipit.Database \
  --startup-project Balenthiran.Snipit.WebApi
```
...
- [ ] No concrete types defined in `Balenthiran.Snipit.Abstractions`
- [ ] Service interface methods return **interfaces**, not concrete types (`IRatio`, not `Ratio`)
- [ ] `Balenthiran.Snipit.Abstractions.csproj` has **zero** `<ProjectReference>` entries to concrete projects
...
- [ ] Domain models in `Balenthiran.Snipit.DomainModels`, data models in `Balenthiran.Snipit.DataModels`
