...
- [ ] No concrete types (classes, records, structs) defined in `Balenthiran.Snipit.Abstractions`
- [ ] Service interface methods return **interfaces**, not concrete types (`IRatio`, not `Ratio`)
- [ ] `Balenthiran.Snipit.Abstractions.csproj` has **zero** `<ProjectReference>` entries to projects with concrete types
...
- [ ] Domain models are in `Balenthiran.Snipit.DomainModels`, data models in `Balenthiran.Snipit.DataModels`
